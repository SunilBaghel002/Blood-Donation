...
const EmergencyBloodDonationPortal = () => {
  ...
  const fetchDisasters = async () => {
    try {
      const res = await fetch("/api/donor/disasters", {
  ...
  const fetchTimeline = async () => {
    try {
      const res = await fetch("/api/donor/timeline", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const handleCommitDonation = async (disaster) => {
    try {
      const res = await fetch(`/api/donor/commit/${disaster.id}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Blockchain-Verified Disaster Relief
                </p>
              </div>
            </div>
      ...
      {donationCommitment && (
        <div className="fixed top-20 right-4 bg-green-500 text-white px-6 py-4 rounded-lg shadow-2xl z-50 animate-slide-in flex items-center space-x-3">
          ...
          <span className="font-medium">
            Commitment registered! Nearest center will contact you.
          </span>
        </div>
      )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {[
            ...
            {
              label: "Units Needed",
              value: stats.unitsNeeded || "0",
              icon: Droplet,
              color: "blue",
            },
            {
              label: "Units Collected",
              value: stats.unitsCollected || "0",
              icon: Activity,
              color: "green",
            },
            {
            ...
            <div
              key={idx}
              className="bg-white rounded-xl shadow-md p-6 border border-gray-100 hover:shadow-xl transition-all transform hover:-translate-y-1"
            >
                    ...
                    <div className="mb-4">
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="text-gray-600">
                          {disaster.unitsCollected} / {disaster.unitsNeeded}{" "}
                          units
                        </span>
                      </div>
                ...
                <div className="bg-gradient-to-br from-blue-100 to-blue-200 rounded-lg h-64 flex items-center justify-center mb-6">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-blue-600">
                      {disasters.length} Active Locations
                    </p>
                  </div>
                </div>
                ...
                {selectedDisaster && (
                  <div className="border-t border-gray-200 pt-4">
                    <h4 className="font-bold text-gray-900 mb-3">
                      Selected: {selectedDisaster.type}
                    </h4>
            ...
            <div className="relative">
              ...
              <div className="space-y-8">
                {filteredTimeline.map((event, idx) => {
                  const IconComponent = AlertCircle; // Map icon strings if needed
                  return (
                    <div
                      key={event.id}
                    ...
                    >
                      ...
                      {/* Event Card */}
                      <div
                        className={`border-2 rounded-lg p-4 hover:shadow-lg transition-all ${getCategoryColor(
                          event.category
                        )}`}
                      >
